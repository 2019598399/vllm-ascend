...
def test_sp_threshold_uses_profile_only_when_user_did_not_set_it():
    pass_config = SimpleNamespace(enable_sp=False, fuse_gemm_comms=True, sp_min_token_num=None)
    config = SimpleNamespace(
        compilation_config=SimpleNamespace(pass_config=pass_config),
        model_config=SimpleNamespace(dtype=torch.bfloat16, get_hidden_size=lambda: 8192),
    ...
    with (
        patch("vllm_ascend.platform.get_current_hardware_profile") as get_profile,
        patch("vllm_ascend.platform._get_default_max_cudagraph_capture_size", return_value=None),
    ):
        NPUPlatform.apply_config_platform_defaults(config)
    assert pass_config.sp_min_token_num == 512
